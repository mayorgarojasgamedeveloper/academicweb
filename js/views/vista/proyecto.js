...
$('document').ready(function() {
  ...
  .done(function(data) {
    ...
    $('#fecha_fin').val(data[0].fecha_fin.substr(0,10));
    $('#colaboradores').val(data[0].colaboradores);
    $('#instituciones').val(data[0].instituciones);
  ...
  $('#menu').on('click', '#btn_eliminar', function() {
    ...
    function(){
      $.ajax({url: `http://localhost:3000/proyecto`, data: {id: id}, method: `delete`})
      .done(function(data) {
  ...
  $('#menu').on('click', '#btn_guardar', function() {
    ...
    $.ajax({url: `http://localhost:3000/proyecto/${id}`, data: objeto, method: `put`})
    .done(function(data) {
      alertify.alert(`Academic`, `El proyecto "${objeto["nombre"]}" ha sido editado exitosamente.`, function(){
        location.reload();
      });
    });
  ...
  $('#btn_descargar').on('click', function() {
    ...
    doc.setFontSize(15);
    doc.text('Proyectos de investigación aplicada y proyectos tecnológicos', 15, 60);

    doc.setFontSize(15);
    doc.text('Nombre del Proyecto: ', 15, 80);
    doc.setFontSize(19);
    doc.text($('#nombre').val(), 15, 90);
    doc.setFontSize(15);
    doc.text('Fecha de Inicio:', 15, 100);
    doc.setFontSize(19);
    doc.text($('#fecha_inicio').val(), 15, 110);
    doc.setFontSize(15);
    doc.text('Fecha de Fin:', 15, 120);
    doc.setFontSize(19);
    doc.text($('#fecha_fin').val(), 15, 130);
    doc.setFontSize(15);
    doc.text('Colaboradores:', 15, 140);
    doc.setFontSize(19);
    doc.text($('#colaboradores').val(), 15, 150);
    doc.setFontSize(15);
    doc.text('Instituciones:', 15, 160);
    doc.setFontSize(19);
    doc.text($('#instituciones').val(), 15, 170);
