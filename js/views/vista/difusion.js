...
$('document').ready(function() {
  ...
  .done(function(data) {
    ...
    $.ajax({url: `http://localhost:3000/reporte1/${id}`, method: `get`})
    .done(function(data) {
      console.log(data);
      // Agregar informacion
      ...
      $('#issn').val(data[0].issn);
      $('#nom_revista').val(data[0].nom_revista);
  ...
  $('#menu').on('click', '#btn_eliminar', function() {
    ...
    function(){
      $.ajax({url: `http://localhost:3000/reporte1`, data: {id: id}, method: `delete`})
      .done(function(data) {
  ...
  $('#menu').on('click', '#btn_guardar', function() {
    ...
    $.ajax({url: `http://localhost:3000/reporte1/${id}`, data: objeto, method: `put`})
    .done(function(data) {
  ...
  $('#btn_descargar').on('click', function() {
    ...
    doc.setFontSize(15);
    doc.text('Artículos de difusión y divulgación', 15, 60);
    ...
    doc.setFontSize(15);
    doc.text('ISSN: ', 15, 160);
    doc.setFontSize(19);
    doc.text($('#issn').val(), 15, 170);
    doc.setFontSize(15);
    doc.text('Nombre de Revista: ', 15, 180);
    doc.setFontSize(19);
    doc.text($('#nom_revista').val(), 15, 190);
