...
$('document').ready(function() {
  ...
  $.ajax({url: `http://localhost:3000/reporte4/${id}`, method: `get`})
  .done(function(data) {
    console.log(data);
    // Agregar informacion
  ...
  $('#menu').on('click', '#btn_eliminar', function() {
    ...
    function(){
      $.ajax({url: `http://localhost:3000/reporte4`, data: {id: id}, method: `delete`})
      .done(function(data) {
  ...
  $('#menu').on('click', '#btn_guardar', function() {
    ...
    $.ajax({url: `http://localhost:3000/reporte4/${id}`, data: objeto, method: `put`})
    .done(function(data) {
      alertify.alert(`Academic`, `La Estadía "${objeto["nombre"]}" ha sido editada exitosamente.`, function(){
        location.reload();
      });
    });
  ...
  $('#btn_descargar').on('click', function() {
    ...
    doc.setFontSize(15);
    doc.text('Estadía en empresas', 15, 60);
    ...
    doc.setFontSize(15);
    doc.text('Nombre Empresa:', 15, 140);
    doc.setFontSize(19);
    doc.text($('#nombre_empresa').val(), 15, 150);
    doc.setFontSize(15);
    doc.text('Nombre Alumno:', 15, 160);
    doc.setFontSize(19);
    doc.text($('#nombre_alumno').val(), 15, 170);
